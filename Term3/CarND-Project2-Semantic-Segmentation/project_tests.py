...
def test_optimize(optimize):
    ...
    correct_labels_ph = tf.placeholder(tf.float32, [None, None, None, num_classes])
    learning_rate_ph  = tf.placeholder(tf.float32)
    logits, train_op, cross_entropy_loss = optimize(layers_output, correct_labels_ph, learning_rate_ph, num_classes)

    _assert_tensor_shape(logits, [2*3*4, num_classes], 'Logits')

    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        sess.run([train_op], {correct_labels_ph: np.arange(np.prod(shape)).reshape(shape), learning_rate_ph: 10})
        test, loss = sess.run([layers_output, cross_entropy_loss], {correct_labels_ph: np.arange(np.prod(shape)).reshape(shape)})
...
def test_train_nn(train_nn):
    ...
    input_images_ph    = tf.placeholder(tf.float32, name='input_image')
    correct_labels_ph  = tf.placeholder(tf.float32, name='correct_label')
    ...
    with tf.Session() as sess:
        parameters = {
            ...
            'input_images_ph'    : input_images_ph,
